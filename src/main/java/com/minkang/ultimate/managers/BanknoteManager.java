...
public class BanknoteManager implements Listener {
    ...
    public ItemStack create(long amount, int qty){
        ...
        meta.setLore(lore);
        if (c.getBoolean("cheque.glow", true)) meta.addEnchant(org.bukkit.enchantments.Enchantment.LUCK_OF_THE_SEA, 1, true);
...
// Identify which hand used the cheque and consume from that exact slot (fix off-hand duplication)
...
// Consume from the hand that triggered the event
